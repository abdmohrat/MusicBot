...
public class CommandEvent
{
    ...
    public void reply(File file, String filename)
    {
        ...
        event.getChannel().sendFiles(FileUpload.fromData(file, name)).queue();
    }
    ...
    {
        String name = filename == null ? file.getName() : filename;
        event.getChannel().sendFiles(FileUpload.fromData(file, name)).setContent(message).queue();
    }
    ...
    public void replyInDm(String message, File file, String filename)
    {
        ...
        else
        {
            event.getAuthor().openPrivateChannel().queue(pc -> {
                String name = filename == null ? file.getName() : filename;
                pc.sendFiles(FileUpload.fromData(file, name)).setContent(message).queue();
            });
        }
    }
    ...
    private void react(String reaction)
    {
        ...
        try
        {
            String formatted = reaction.replaceAll("<a?:(.+):(\\d+)>", "$1:$2");
            event.getMessage().addReaction(Emoji.fromFormatted(formatted)).queue();
        }
    ...
    {
        return event.isFromType(ChannelType.PRIVATE) ? event.getChannel().asPrivateChannel() : null;
    }
    ...
    {
        return event.isFromType(ChannelType.TEXT) ? event.getChannel().asTextChannel() : null;
    }
