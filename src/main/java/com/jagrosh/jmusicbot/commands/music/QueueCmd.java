...
public class QueueCmd extends MusicCommand 
{
    ...
    public void doCommand(CommandEvent event)
    {
        ...
        int itemsPerPage = 10;
        int pages = (int)Math.ceil(list.size() / (double)itemsPerPage);
        ...
        sb.append(getQueueTitle(ah, event.getClient().getSuccess(), list.size(), total, settings.getRepeatMode(), settings.getQueueType()));
        sb.append("\n").append("Page ").append(pagenum).append("/").append(pages).append("\n");
        for(int i=start; i<end; i++)
        {
            sb.append('`').append(i+1).append("` ").append(FormatUtil.filter(list.get(i).toString())).append("\n");
        }
